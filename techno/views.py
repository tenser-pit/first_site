...
class TCategory(DataMixin, ListView):
    ...
    template_name = 'techno/category.html'
    # paginate_by = 2
    ...
    def get_queryset(self):
        return Techno.objects.filter(cat=Category.objects.get(slug=self.kwargs['cat_slug']).id)


class TGoods(DataMixin, ListView):
    model = Techno
    context_object_name = 'goods'
    template_name = 'techno/goods.html'

    def get_context_data(self, *, object_list=None, **kwargs):
        context = super().get_context_data(**kwargs)
        c_def = self.get_user_context(title='Товар:' + Techno.objects.get(slug=self.kwargs['goods_slug']).title,
                                      goods_slug=self.kwargs['goods_slug'])
        return dict(list(context.items()) + list(c_def.items()))

    def get_queryset(self):
        return Techno.objects.filter(slug=self.kwargs['goods_slug'])


class TLogin(DataMixin, LoginView):
    form_class = LoginUserForm
    template_name = 'techno/login.html'
...
class TRegister(DataMixin, CreateView):
    ...
    template_name = 'techno/register.html'
...
class TCart(LoginRequiredMixin, DataMixin, TemplateView):
    template_name = 'techno/cart.html'
...
class TProfile(LoginRequiredMixin, DataMixin, TemplateView):
    template_name = 'techno/profile.html'
    raise_exception = True
...
def about(request):
    return render(request, 'techno/about.html')


def contact(request):
    return render(request, 'techno/contact.html')
